...
def main():
    ...
    while True:
        ...
        if len(todos_pontos) == 21: #so entra no if se ele detectar os 21 pontos corretamente
            ...
            y_dedo_indicador1 = todos_pontos[8][2]
            y_junta_inferior1 = todos_pontos[6][2]

            y_dedo_medio = todos_pontos[12][2]
            y_junta_medio = todos_pontos[10][2]

            y_dedo_anelar = todos_pontos[16][2]
            y_junta_anelar = todos_pontos[14][2]

            if y_dedo_indicador1 < y_junta_inferior1: #se a ponta do dedo for maior que a base
                cv2.putText(frame, "Dedo indicador", (50, 100), cv2.FONT_HERSHEY_SIMPLEX,#texto, posicao do texto, tamanho da vonte, cor do texto e espessura da fonte
                            1, (0, 255, 255), 5)
                print(f"Posição do dedo indicador: x={todos_pontos[8][1]}, y={y_dedo_indicador1}") #exibe as coordenadas do dedo indicador
                arduino.write(b"1")

            if y_dedo_medio < y_junta_medio:
                cv2.putText(frame, "Dedo medio", (50, 200), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 255, 255), 5)
                print(f"Posição do dedo medio: x={todos_pontos[8][1]}, y={y_dedo_medio}")
                arduino.write(b"2")

            if y_dedo_anelar < y_junta_anelar:
                cv2.putText(frame, "Dedo anelar", (50, 300), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 255, 255), 5)
                print(f"Posição do dedo anelar: x={todos_pontos[8][1]}, y={y_dedo_medio}")
                arduino.write(b"3")

            '''dedos_levantados = 0

            if y_dedo_indicador1 < y_junta_inferior1:
                dedos_levantados += 1
            ...
            cv2.putText(frame, f"{dedos_levantados} dedos", (50, 100),
            cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 5)

            print(f"Indicador - x: {todos_pontos[8][1]}, y: {y_dedo_indicador1}")'''
